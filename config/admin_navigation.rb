SimpleNavigation::Configuration.run do |navigation|
  ...
  navigation.items do |primary|
    ...
    primary.item :authors, author_nav_item_caption, admin_users_path, if: ->{administrator_signed_in?} do |authors|
      authors.auto_highlight = false
      authors.dom_class = 'nav nav-tabs'
      authors.item :pending, 'Pending', admin_users_path, highlights_on: -> { author_path?(Author.PENDING) }
      authors.item :approved, 'approved', admin_users_path(status: :approved), highlights_on: -> { author_path?(Author.APPROVED) }
      authors.item :rejected, 'Rejected', admin_users_path(status: :rejected), highlights_on: -> { author_path?(Author.REJECTED) }
    end
